...
public static class SecureHeadersMiddlewareBuilder
{
    ...
    /// </summary>
    /// <param name="maxAge">
    /// The time, in seconds, that the browser should remember that this
    ...
    /// based on the OWASP best practices values for HSTS.
    /// </remarks>
    ...
    /// Applicable values are: deny; sameorigin; and allow from
    /// </param>
    /// <param name="domain">
    /// If allow from is supplied, this optional parameter describes the domain in which
    /// our site is permitted to be loaded within a frame
    /// </param>
    /// <remarks>
    /// If no value for <param name="xFrameOption"/> is provided, then default one will be
    /// used. This default value is based on the OWASP best practices value for X-Frame-Options.
    /// </remarks>
    ...
    /// An option uri to report any XSS filter violation to. This parameter is optional
    ...
    /// be used. This default is based on the OWASP best practices for XSS Protection
    /// <remarks></remarks>
    ...
    ///  - an ObjectSrc of "self"
    /// </remarks>
    public static SecureHeadersMiddlewareConfiguration UseLabCibeContentSecurityPolicy
    (this SecureHeadersMiddlewareConfiguration config)
    {
        config.UseContentSecurityPolicy = true;
        ...
        config.SetCspUris(
            ContentSecurityPolicyHelpers.CreateCustomDirectives(CsdsContentPolicy.Base),
            CspUriType.Base);

        config.SetCspUris(
            ContentSecurityPolicyHelpers.CreateCustomDirectives(CsdsContentPolicy.Style),
            CspUriType.Style);

        config.SetCspUris(
            ContentSecurityPolicyHelpers.CreateCustomDirectives(CsdsContentPolicy.Script),
            CspUriType.Script);        

        config.SetCspUris(
            ContentSecurityPolicyHelpers.CreateCustomDirectives(CsdsContentPolicy.Font),
            CspUriType.Font);

        config.SetCspUris(
            ContentSecurityPolicyHelpers.CreateCustomDirectives(CsdsContentPolicy.FrameAncestor),
    ...
    /// <summary>
    /// Adds the Feature-Policy header to all responses
    /// That would prevent from injecting local script that uses the webcam, or some other client-side feature.
    /// for that site from going unnoticed
    /// </summary>
    /// <exception cref="ArgumentException">
    /// An ArgumentException is thrown when no Report URI is supplied
    /// </exception>
    public static SecureHeadersMiddlewareConfiguration UseFeaturePolicy
        (this SecureHeadersMiddlewareConfiguration config)
    {
        config.UseFeaturePolicy = true;
        config.FeaturePolicy = new FeaturePolicy();
        return config;
    }

    /// <summary>
    /// Thanks to Stack Overflow user rawb for this description:
